-- SSOT Migration - добавляет таблицы к существующей БД
-- Выполнять для каждой базы данных проекта
...
CREATE INDEX IF NOT EXISTS idx_dedup_hash ON dedup_registry(content_hash);
...
CREATE INDEX IF NOT EXISTS idx_workspace_member ON workspace_memories(team_member);
...
SELECT id, 'memory'::varchar as source_type, content, embedding, user_id, project_id, created_at,
       jsonb_build_object('memory_type', memory_type, 'domain', domain) as metadata
FROM memories WHERE NOT is_deleted
UNION ALL
SELECT id, 'episode'::varchar as source_type, content, embedding, user_id, project_id, created_at,
       jsonb_build_object('name', name) as metadata
FROM graphiti_episodes
UNION ALL
SELECT id, 'entity'::varchar as source_type, COALESCE(summary, name) as content, embedding, 
       NULL::varchar as user_id, NULL::varchar as project_id, created_at,
       jsonb_build_object('name', name, 'entity_type', entity_type) as metadata
FROM graphiti_entities
UNION ALL
SELECT c.id, 'kb_chunk'::varchar as source_type, c.content, c.embedding,
       NULL::varchar as user_id, d.project_id, c.created_at,
...
BEGIN
    RETURN QUERY
    SELECT v.id, v.source_type, v.content,
...
SELECT 'SSOT Migration completed successfully';
